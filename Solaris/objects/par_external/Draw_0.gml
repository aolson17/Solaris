...
if room = rm_builder{
	...
	if sprite_index != -1{
		...
		//draw_sprite_ext(sprite_index,0,adj_x,adj_y,1,1,my_ship.angle+image_angle,c_white,image_alpha)
		
		// Rotate the adjusted x and y around the center of the surface
		var adj_x_surf = scr_x_rotated_around_point(adj_x-surface_get_width(surf)/2,adj_y-surface_get_height(surf)/2,adj_x,adj_y,my_ship.angle)
		var adj_y_surf = scr_y_rotated_around_point(adj_x-surface_get_width(surf)/2,adj_y-surface_get_height(surf)/2,adj_x,adj_y,my_ship.angle)
		
		draw_surface_ext(surf,adj_x_surf,adj_y_surf,1,1,my_ship.angle,c_white,1)
	}
}
